...
NSString * TheHorseStable$ModuleRequireFormat = @"(function(exports){"
		"var __OXP=exports;var module={'exports':exports};%@;\n"
		"if(module.exports !== __OXP){return module.exports;}"
		"return exports;})({})";


@implementation TheHorseStableObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_thehorsestable);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create TheHorseStable global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "TheHorseStable" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_thehorsestable = [[TheHorseStableObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_thehorsestable];
	
	NSString *_thehorsestableNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _thehorsestableNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_thehorsestable valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:TheHorseStable$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.thehorsestable.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.thehorsestable.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
